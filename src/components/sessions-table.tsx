...
import { getSessions, PaginatedFilterRequest, Session } from "@/services/interaction/interaction.ts";
...
export default function SessionsTable({ tableProps, filter }: {
    tableProps: {
        id: boolean,
        logReader: boolean,
        logUser: boolean,
    ...
    const [data, setData] = useState<{
        items: (Session & { timeElapsed: string })[];
    ...
    useEffect(() => {
        const fetchLogs = async () => {
            const logs = await getSessions({ ...filter, page: currentPage });
            ...
            const updatedLogs = logs.items.map((log) => ({
                ...
                timeElapsed: calculateTimeElapsed(log.startDate, log.endDate),
            }));
                ...
                <TableBody>
                    {data.items.map((log) => (
                        <TableRow key={log.id}>
                            ...
                            {tableProps.endDate && <TableCell><DateBadge variant="exit" dateTime={log.endDate} /></TableCell>}
